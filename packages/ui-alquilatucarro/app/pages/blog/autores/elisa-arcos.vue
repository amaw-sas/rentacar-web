    ...
    <section class="bg-linear-to-b from-footer-from to-footer-to px-4 py-12 sm:px-6 sm:py-16 lg:px-8">
      <div class="max-w-4xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-8">
          ...
          >
          <div>
            <p class="text-brand-200 font-semibold mb-2">{{ AUTHOR.jobTitle }}, Alquila tu Carro</p>
            <h1 class="heading-page text-white">Elisa Arcos</h1>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="bg-white px-4 py-12 sm:px-6 lg:px-8">
      <div class="max-w-4xl mx-auto">
        <div class="prose prose-lg prose-gray max-w-none">
          <p v-for="(paragraph, index) in AUTHOR.bio" :key="index">{{ paragraph }}</p>
        </div>

        <div class="mt-12 border-t border-gray-200 pt-10">
          <h2 class="heading-section text-gray-900">Artículos de Elisa Arcos</h2>
...
import { AUTHOR } from './elisa-arcos.author'

const { franchise } = useAppConfig()
const authorPhotoFiles = import.meta.glob(
  './images/elisa-arcos.{avif,webp,jpg,jpeg,png}',
  { eager: true, query: '?url', import: 'default' },
) as Record<string, string>
const authorPhoto = Object.values(authorPhotoFiles)[0]

const { data: postsData } = await useAsyncData('author-elisa-arcos-posts', async () => {
...
const canonicalUrl = `${franchise.website}/blog/autores/elisa-arcos`
const pageTitle = `Elisa Arcos — ${AUTHOR.jobTitle}, Alquila tu Carro`
...
useSeoMeta({
  ...
  ogTitle: pageTitle,
  ogDescription: AUTHOR.metaDescription,
  ogType: 'profile',
  ogUrl: canonicalUrl,
})
