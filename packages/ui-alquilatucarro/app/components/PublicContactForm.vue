  ...
  <form ref="formEl" class="space-y-5" novalidate @submit.prevent="submit" @input="onEdit" @change="onEdit">
    <div v-for="f in fields" :key="f.name">
      ...
      <template v-if="f.type === 'checkbox'">
        <div class="flex items-start gap-3">
          <input
            ...
            class="mt-1 size-5 shrink-0 rounded border-gray-300 text-red-600 focus:ring-2 focus:ring-red-500"
          >
          <label :for="`f-${f.name}`" class="text-sm text-gray-700 leading-relaxed">
            {{ f.label }}
            <span v-if="f.required" class="text-red-600" aria-hidden="true">*</span>
            <span v-if="f.required" class="sr-only">(obligatorio)</span>
          </label>
        </div>
        <p v-if="errors[f.name]" :id="`e-${f.name}`" class="mt-1.5 text-sm text-red-700">
          {{ errors[f.name] }}
        </p>
      </template>
        ...
        <div class="flex flex-wrap gap-x-6 gap-y-3">
          <div v-for="opt in f.options || []" :key="opt" class="flex items-center gap-2">
            <input
              ...
              type="checkbox"
              :value="opt"
              class="size-5 rounded border-gray-300 text-red-600 focus:ring-2 focus:ring-red-500"
            >
      ...
      <textarea
        ...
        :aria-invalid="Boolean(errors[f.name])"
        :aria-describedby="errors[f.name] ? `e-${f.name}` : undefined"
        class="w-full rounded-xl border px-4 py-3 text-gray-900 bg-white transition-colors focus:outline-none focus:ring-2 focus:ring-red-500"
        :class="errors[f.name] ? 'border-red-600' : 'border-gray-300'"
      />
      <input
        v-else
        :id="`f-${f.name}`"
    ...
    <button
      ...
      class="inline-flex items-center gap-2 px-8 py-3.5 rounded-full bg-red-600 text-white font-semibold shadow-lg shadow-black/15 transition-all duration-200 hover:bg-red-700 hover:-translate-y-0.5 disabled:opacity-60 disabled:cursor-not-allowed disabled:translate-y-0"
    >
    ...
    <p
      role="alert"
      :class="failed ? 'rounded-xl bg-red-50 border border-red-200 text-red-800 px-4 py-3' : 'sr-only'"
    >
      {{ failed }}
    </p>
  </form>
