<template>
  ...
  >
    ...
    <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 md:py-12 w-full">
      <div class="home-hero-grid grid lg:grid-cols-2 gap-3 lg:gap-10 items-center">
        <div class="home-hero-copy text-center lg:text-left">
          <!-- Trust signal: Google reviews star badge (parity with the city hero). -->
          <div
            class="home-hero-review-row hero-review-row flex flex-row space-x-0.5 justify-center lg:justify-start items-center text-sm text-white mb-3"
          >
            <IconsStarIcon v-for="i in [1, 2, 3, 4, 5]" :key="i" cls="w-3.5 h-3.5 md:w-4 md:h-4" />
            <span class="hero-review-label ml-2">Reseñas en Google</span>
          </div>
          ...
          <h1
            class="home-hero-title text-3xl sm:text-4xl lg:text-5xl xl:text-6xl font-extrabold font-heading text-white leading-[1.1]"
          >
          ...
          <!-- Single CTA: WhatsApp. The former secondary pill only scrolled to
               ...
               actually converts. -->
          <div class="home-hero-cta-row mt-6 flex flex-row items-stretch gap-3 justify-center lg:justify-start">
            <a
              :href="franchise.whatsapp"
              target="_blank"
              rel="noopener noreferrer"
              aria-label="Contáctanos por WhatsApp"
              class="inline-flex items-center justify-center gap-2 px-6 sm:px-7 py-3.5 text-base font-semibold rounded-full bg-whatsapp text-black hover:bg-whatsapp-hover shadow-lg shadow-black/15 hover:shadow-xl transition-[background-color,box-shadow] duration-200"
            >
              <svg
              ...
              <span class="hero-contact-label">WhatsApp</span>
            </a>
          </div>
        </div>
