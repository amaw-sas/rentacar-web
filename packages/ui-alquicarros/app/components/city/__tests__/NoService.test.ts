...
describe('CityNoService — alquicarros no usa rojo', () => {
  const src = readFileSync(resolve(__dirname, '../NoService.vue'), 'utf8')

  // Regla de marca ya fijada en cities.test.ts y contact.test.ts: alquicarros es brand-*, el rojo
  // es de las otras dos. Copiar el aviso entre marcas sin mirar la habria roto.
  it('no usa clases red-N de Tailwind', () => {
    expect(src).not.toMatch(/\b(bg|text|border|from|to|via|ring|shadow|fill)-red-\d/)
  })

  it('usa el acento brand-*', () => {
    expect(src).toMatch(/bg-brand-\d00/)
  })
})
