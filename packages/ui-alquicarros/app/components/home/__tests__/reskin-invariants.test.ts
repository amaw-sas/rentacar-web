...
/**
 * Stacking contract (runtime bug, ported from alquilame):
 *   GIVEN the home scrolled a few px past the top
 *   WHEN  the sticky header (default.vue → UHeader `sticky top-0 z-50`) overlaps
 *         the announcement bar, which lives inside <main> and therefore AFTER
 *         the header in the DOM
 ...
 * the header and clipped the logo + the menu toggle.
 */
describe('reskin — announcement bar stays under the sticky header', () => {
  ...
  it('gives the bar a strictly lower z-index than the sticky header', () => {
    const bar = readFileSync(join(HOME, 'AnnouncementBar.vue'), 'utf-8')
