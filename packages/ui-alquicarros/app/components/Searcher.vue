...
// Submit destination — alquicarros PATH model (routing independence, directiva).
// `/reservas` es la única superficie de reserva y lleva la búsqueda en el PATH
// (espejo de la estructura buscar-vehiculos, sin el segmento [city]). Las páginas de
// resultados leen estos params vía useSearchByRouteParams y montan el wizard.
// `buscar-vehiculos` ya no existe en alquicarros, así que no hay rama route.params.city.
// referido, cuando existe, es el segmento inicial (espeja el árbol de páginas).
// Los valores se codifican por segmento (la hora 12h lleva un ':').
...
// Block submit until a resolved pickup-branch slug + both dates exist (otherwise the
// PATH would be half-empty and the search guard would no-op). alquicarros has a single
// surface (/reservas, no route.params.city), so the guard is uniform.
...
const calendarUIConfig = {
    ...
    cellTrigger: '!text-gray-900 !font-semibold data-[disabled]:!text-gray-400 data-[disabled]:!opacity-50 data-[unavailable]:!text-gray-400 data-[unavailable]:!opacity-50 data-[outside-view]:!bg-gray-100'
};
...
const mobileCalendarUIConfig = {
    ...
    gridBody: 'gap-y-3',
    headCell: 'text-base',
    cellTrigger: '!text-gray-900 !font-semibold !size-12 !text-lg !m-0 data-[disabled]:!text-gray-400 data-[disabled]:!opacity-50 data-[unavailable]:!text-gray-400 data-[unavailable]:!opacity-50 data-[outside-view]:!bg-gray-100',
};
...
<style scoped>
/* El segmento de año (4 dígitos) se partía en dos líneas en campos estrechos
   del input-date (ancho de segmento fijo + white-space normal). Mantener cada
   segmento en una sola línea y dejar que el año use el ancho que necesita. */
:deep([data-reka-date-field-segment]) {
  white-space: nowrap;
}
:deep([data-reka-date-field-segment][data-segment='year']) {
  width: auto;
}
</style>
