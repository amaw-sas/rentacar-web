...
- `mode === 'landing'`: **sin** `<Searcher>`; en la columna del engine va el CTA "Reservar ahora" → `/reservas`. **Conserva** h1 city-SEO ("Alquiler de carros en {city}") + pin #41 (`<span aria-hidden>` inerte, copy-to-WhatsApp). El ancla `#searcher` se mantiene como `<div>` vacío (sigue siendo target válido del scroll, aunque sin form) **o** se elimina y el contact apunta a `/reservas` (ver acoplamiento abajo).
...
`CityPage.vue` recibe `mode` y lo reenvía a `<CityHero :city :mode>`. Cada page file lo fija:
- `app/pages/[city]/index.vue` → `<CityPage :city mode="landing" />`
- las 4 rutas `…/buscar-vehiculos/…/index.vue` (con y sin `categoria`, con y sin `referido`) → `<CityPage :city mode="results" />`
