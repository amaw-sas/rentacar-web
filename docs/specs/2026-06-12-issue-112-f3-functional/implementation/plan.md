...
1. **Searcher: derivación de ciudad** | S | deps: none
   ...
   - AC: override local tras copiar `searchLinkParams` (sin tocar `useSearch`/`packages/logic`); `data-testid` `pickup/return-location-test` intactos; test guard: `city = pickupBranch.city` cuando ruta sin city, sin cambio cuando hay city; `git diff packages/logic` vacío.
...
5. **Header: CTA Reservar** | S | deps: none
   ...
   - AC: `default.vue` con CTA `to="/reservas"` en ambos viewports; test de chrome (si aplica) cubre presencia + destino; sin `bg-gradient-to-`.
...
6. **Reskin grilla de resultados** | M | deps: none
   ...
   - AC: testids intactos; `bg-linear` (no `bg-gradient-to-`); headings Jakarta legibles (`[--ctx-text-primary:#fff]` en fondos oscuros); slideover único (#65) sin cambio de comportamiento; test cubre testids + bg-linear.
...
7. **Reskin slideover de reserva** | M | deps: none
   ...
   - AC: testids intactos; styling marca; `ReservationForm`/`ReservationResume` tests existentes verdes (ajustar a styling, no debilitar contrato); sin `bg-gradient-to-`.
